...
use slop_veil::zk::{ZkIopCtx, ZkProof};
...
/// VEIL zero-knowledge masking proof for post-quantum ZK-STARKs.
///
/// Contains the masked commitment and the full ZK proof (which includes PCS
/// evaluation proofs, constraint proofs, and transcript data internally).
/// When present in a [`ShardProof`], the verifier must check both the standard
/// evaluation proof AND this VEIL proof.
#[derive(Clone, Serialize, Deserialize)]
#[serde(bound(
    serialize = "GC: ZkIopCtx, GC::PcsProof: Serialize",
    deserialize = "GC: ZkIopCtx, GC::PcsProof: serde::de::DeserializeOwned"
))]
pub struct VeilMaskingProof<GC: ZkIopCtx> {
    /// VEIL masked commitment over the stacked execution traces.
    pub masked_commitment: GC::Digest,
    /// Full ZK proof generated by `ZkProverCtx::prove()`. Contains:
    /// - PCS evaluation proofs (stacked Basefold)
    /// - Linear/multiplicative constraint proofs
    /// - Transcript data for Fiat-Shamir
    pub zk_proof: ZkProof<GC>,
    /// The number of encoding variables used for VEIL PCS initialization.
    pub num_encoding_vars: u32,
    /// The log2 of the number of stacked polynomials (columns).
